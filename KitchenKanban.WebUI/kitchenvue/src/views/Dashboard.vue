                ...
                <div class="kb-main">
                    <div class="kb-column">
                        ...
                        <div class="ticket-sec">
                            <div id="ordermaindiv" class="ticket-main" v-bind:key="index" v-for="(order,index) in neworders.orders">
                                <div class="order-detail" @click="toggleOrderDetails(order,index)">
                                    <div>
                                        <div class="float-left">{{order.orderId}} [ {{order.orderDate | formatTime}}]</div>
                                        <div class="float-right" v-if="order.orderType==1">DineIn</div>
                                        <div class="float-right" v-if="order.orderType==2">Delivery</div>
                                        <div class="clearfix"></div>
                                    </div>
                                    <div>
                                        <div class="float-left">{{order.customerName}}</div>
                                        <div class="float-right">{{order.customerContactNumber}}</div>
                                        <div class="clearfix"></div>
                                    </div>
                                </div>
                                <div :id="'orderdetailsdiv' + index" class="order-description" style="display:none;">
                                    <table class="order-tbl">
                                        <tbody>
                                            ...
                                            <tr v-bind:key="orderline.itemId" v-for="orderline in orderlines.orderLines">
                                                <td>{{orderline.itemId}}</td>
                                                <td class="text-center">{{orderline.orderQuantity}}</td>
                                                <td class="text-right">{{orderline.itemId}}</td>
                                            </tr>
                                        </tbody>
                                    </table>
                                    <div class="top6">
                                        <div class="float-right">
                                            <div class="dropup tkt-left-btn">
                                                <button class="move-order-btn dropdown-toggle" type="button" data-toggle="dropdown">
                                                    Move To
                    ...
                    <div class="kb-column">
                        <div class="kb-column-header">Ready for Delivery<span class="order-count">03</span></div>
                        <div class="ticket-sec">
    ...
    getKanboard() {
      ...
      dashBoardService
        ...
        .then(response => {
            ...
            this.neworders=response.data.filter(item=>item.flowOrder===1)[0];
            this.beingpreparedorders=response.data.filter(item=>item.flowOrder===2);
            this.preparedorders=response.data.filter(item=>item.flowOrder===3);
            this.packingorders=response.data.filter(item=>item.flowOrder===4);
            this.readyorders=response.data.filter(item=>item.flowOrder===5);
